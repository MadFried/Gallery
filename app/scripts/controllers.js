...
.controller('MenuController', ['$scope', 'menuFactory', 'favoriteFactory', function ($scope, menuFactory, favoriteFactory) {
    ...
    $scope.addToFavorites = function(workid) {
        console.log('Add to favorites', wordid);
        favoriteFactory.save({_id: workid});
        $scope.showFavorites = !$scope.showFavorites;
    };
}])
...
.controller('WorkDetailController', ['$scope', '$state', '$stateParams', 'menuFactory', 'commentFactory', function ($scope, $state, $stateParams, menuFactory, commentFactory) {

    $scope.work = {};
    $scope.showWork = false;
    $scope.message = "Loading ...";

    $scope.work = menuFactory.get({
            id: $stateParams.id
        })
        .$promise.then(
            function (response) {
...
.controller('FavoriteController', ['$scope', '$state', 'favoriteFactory', function ($scope, $state, favoriteFactory) {
    ...
    $scope.deleteFavorite = function(workid) {
        console.log('Delete favorites', workid);
        favoriteFactory.delete({id: workid});
        $scope.showDelete = !$scope.showDelete;
        $state.go($state.current, {}, {reload: true});
    };
}])
